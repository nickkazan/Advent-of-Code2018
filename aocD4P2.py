...
def sleepCheck(arr):
    ...
    for a in range(len(arr)):
        ...
        timeDict[timeStamp] = action;

    asleep = False;
    guardDict = {};
    guardID = '';

    for b in range(151801010000, 151812312400):
        if b in timeDict:
            if timeDict[b] == 'up':
                asleep = False;
                pass;
            elif timeDict[b] == 'asleep':
                asleep = True;
                guardDict[b] = guardID;
            else:
                guardID = timeDict[b];
    ...
    result = {};
    maxTime = defaultdict(list);
    for key, value in guardDict.items():
        maxTime[value].append((str(key))[10:]);
        if value in result:
            result[value] += 1;
        else:
            result[value] = 1;
    ...
    for d in result.keys():
        if result[d] > max1:
            max1 = result[d];
            guard = d
    for check in maxTime:
        maxNum = max(maxTime[check],key=maxTime[check].count);
        count = maxTime[check].count(maxNum)
        if count > max2:
            max2 = count;
            maxFinal = maxNum;
            guard2 = check;

    mostCommonTime = max(maxTime[guard],key=maxTime[guard].count);
    mostCommonTime = int(mostCommonTime);
    guard2 = int(guard2[1:]);
    guard = int(guard[1:]);
    maxFinal = int(maxFinal);
    return guard2 * maxFinal;


print(sleepCheck(["[1518-09-28 00:56] wakes up",
