...
def positionTrack(arr):
    coords = [];
    mapResult = [];
    flag = True;
    removal = "p|o|s|i|t|i|o|n|=|<|,|>|v|e|l|o|c|i|t|y";

    for a in range(len(arr)):
        data = re.sub(removal, "", arr[a]);
        data = data.split();
        positionX = int(data[0]);
        positionY = int(data[1]);
        velocityX = int(data[2]);
        velocityY = int(data[3]);
        coords.append(Coordinate(positionX,positionY,velocityX,velocityY));
    ...
    while flag:
        ...
        for coord in coords:
            coord.xP += coord.xV;
            coord.yP += coord.yV;
            locationCheck[(coord.xP,coord.yP)] = True;
            #print('x: ', coord.xP, ' y: ', coord.yP, ' VelX: ', coord.xV, ' VelY: ', coord.yV);
        #check the updated positions
        for coord in coords:
            counter = 0;
            if (coord.xP,coord.yP + 1) not in locationCheck:
                counter += 1;
            if (coord.xP + 1,coord.yP + 1) not in locationCheck:
                counter += 1;
            if (coord.xP + 1,coord.yP) not in locationCheck:
                counter += 1;
            if (coord.xP,coord.yP - 1) not in locationCheck:
                counter += 1;
            if (coord.xP - 1,coord.yP - 1) not in locationCheck:
...
print(positionTrack(["position=< 11153,  22033> velocity=<-1, -2>",
